...
FoxLedger / 狐狐记账 Web/PWA 当前生产验收基线为 **V3.0 狐狐对话记账版**。V3.0 已于 2026-08-13 完成 M0–M5 代码、本地生产构建、Vercel 生产部署和真机 PWA 更新验收，现已正式收口。后续修改仍不能从本地代码状态推断生产状态。

V3.1 M0–M4 已完成验收并保存独立提交；`fox-chat` 已部署。M5 的本地自动化、桌面/Pixel 7 Playwright、Axe 无障碍、离线应用壳和受控真实只读问账已通过，移动端发送布局跳动已修复；V3.1 静态前端尚未推送发布，生产站点仍以 V3.0 为准，生产与真机 PWA 更新验收待完成。
...
- V3.1 M0–M4 已完成验收；M5 本地与受控浏览器验收已通过，生产发布和真机 PWA 更新仍需以实际结果为准。
- 在生产部署、服务器产物和真机 PWA 更新完成前，不得把 V3.1 写成生产已收口。
