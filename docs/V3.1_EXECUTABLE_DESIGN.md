...
- 文档性质：已确认产品边界后的实施设计；M0–M3 已完成本地实现，M4–M5 尚未实现
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0–M3 已完成并通过验收，尚未推送或发布 V3.1 前端；`fox-chat` 已部署用于验收，生产站点前端仍为 V3.0
...
当前状态：完整 RLS 查询、完整统计包、最多 500 条五字段明细、第二次 AI、严格 grounded answer、metric/evidence refs 校验、服务端数字替换、第二次 AI 失败时统计卡降级、normalized 内存连续追问、前端问账卡、代码渲染依据及白名单账单筛选跳转已完成实现、自动化回归和受控人工验收。`fox-chat` 已部署用于验收，V3.1 前端尚未推送或发布；M4 未开始。
