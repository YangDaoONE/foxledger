...
- 文档性质：已确认产品边界后的实施设计；M0–M1 已完成本地代码和自动化检查，M2–M5 尚未实现
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0“共享统计与契约测试”和 M1“安全只读数据层”已在本地完成，尚未推送或发布；生产站点仍为 V3.0
...
当前状态：Edge JWT、邮箱白名单、环境变量和 OpenAI client 已抽为公共模块；用户 JWT + publishable key + RLS 的完整稳定分页、白名单筛选、正式统计、商家分组、比较和最多 500 条安全明细选择已完成本地实现与自动化检查。任一页或行校验失败都整体报错，不返回部分统计；未新增 `fox-chat` 或数据库写路径，未开始 M2。
