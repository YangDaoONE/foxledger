...
- 文档性质：已确认产品边界后的实施设计；M0–M2 已完成本地代码和自动化检查，M3–M5 尚未实现
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0–M2 已在本地完成，尚未推送或发布；生产站点仍为 V3.0
...
当前状态：Edge JWT、邮箱白名单、环境变量和 OpenAI client 已抽为公共模块；用户 JWT + publishable key + RLS 的完整稳定分页、白名单筛选、正式统计、商家分组、比较和最多 500 条安全明细选择已完成本地实现与自动化检查。任一页或行校验失败都整体报错，不返回部分统计；M1 本身未新增 `fox-chat` 或数据库写路径，M2 已在后续独立批次完成。
...
当前状态：`fox-chat` 第一次 AI、四类严格 intent union、V3.0 交易清洗复用、normalized query plan 校验、白名单澄清/不支持 key 和强制记账/问账纠错已完成本地实现与自动化检查。当前不接受非空连续追问 context，不调用 M1 数据读取或第二次 AI，不生成问账回答，未开始 M3。
