...
- 文档性质：已确认产品边界后的实施设计；M0–M4 已完成验收，M5 本地与浏览器验收已完成，等待生产与真机收口
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0–M4 已完成验收，M5 的本地自动化、桌面/Pixel 7 Playwright、Axe、离线应用壳和受控真实问账已通过；`fox-chat` 已部署，V3.1 静态前端尚未发布，等待生产与真机安装态 PWA 更新验收
...
当前状态：完整 RLS 查询、完整统计包、最多 500 条五字段明细、第二次 AI、严格 grounded answer、metric/evidence refs 校验、服务端数字替换、第二次 AI 失败时统计卡降级、normalized 内存连续追问、前端问账卡、代码渲染依据及白名单账单筛选跳转已完成实现、自动化回归和受控人工验收。`fox-chat` 已部署用于验收，M0–M3 已推送；V3.1 静态前端尚未发布。
...
当前状态：统一设计 token、共享页面头/指标卡/区块、登录与五个主路由表现迁移、品牌橙与支出语义色分离、同步缓存行数/最近成功时间/失败原因/手动重试、可见 focus、`aria-live`、`aria-busy`、reduced-motion 和 Safe Area 适配已完成。未修改同步算法、交易 API、统计口径、数据库 schema、RLS 或 AI 数据边界；M4 已通过验收并保存为提交 `3a7b11b`，尚未推送或发布。
...
当前状态：新增 15 条系统 Chrome 桌面、Pixel 7、Axe 与 PWA Playwright 流程；修复移动端输入框失焦导致发送按钮布局跳动，并将品牌/财务/导航颜色调整到 WCAG AA；受控真实账号只读问账已返回可信范围、代码统计和五字段依据，未触发写入。`lint`、`typecheck`、33 个测试文件/157 项测试、`build`、Playwright、`verify:v3` 与依赖审计已通过；等待推送、生产产物与真机安装态 PWA 更新验收。
