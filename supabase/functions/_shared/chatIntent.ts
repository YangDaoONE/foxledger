  ...
  return [
    {
      ...
      content: [
        ...
        "For each record_transaction candidate, raw_text must keep the original date expression and amount that belong to that transaction.",
        `For record_transaction, resolve compact date/amount notation from context: "今天，7.6吃饭" means amount 7.6 on ${todayIsoDate}; "7.6吃饭花了76" means date ${todayIsoDate.slice(0, 4)}-07-06 and amount 76. A decimal followed by a currency unit is always an amount.`,
        "If a bare compact decimal such as 7.6 has no date context and no second amount, return a record_transaction candidate with needs_clarification=true instead of guessing whether it is a date or amount.",
