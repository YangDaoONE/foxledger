...
- 文档性质：已确认产品边界后的实施设计；M0–M3 已完成验收，M4 已完成本地实现并等待验收，M5 尚未实现
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0–M3 已完成验收并推送，M4 已完成本地实现和自动化检查并等待验收；`fox-chat` 已部署用于验收，V3.1 静态前端尚未发布，生产站点前端仍为 V3.0
...
当前状态：统一设计 token、共享页面头/指标卡/区块、登录与五个主路由表现迁移、品牌橙与支出语义色分离、同步缓存行数/最近成功时间/失败原因/手动重试、可见 focus、`aria-live`、`aria-busy`、reduced-motion 和 Safe Area 适配已完成本地实现。未修改同步算法、交易 API、统计口径、数据库 schema、RLS 或 AI 数据边界；33 个测试文件、156 项测试及 `lint`、`typecheck`、`build`、`verify:v3` 已通过，等待用户验收，尚未发布。
