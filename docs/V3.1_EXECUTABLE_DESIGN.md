...
- 文档性质：已确认产品边界后的实施设计；M0–M5 已于 2026-08-17 完成代码、自动化、部署、服务器和真机验收，V3.1 已正式收口
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0–M5 代码已推送，`fox-chat` 和静态前端均已部署；本地自动化、桌面/Pixel 7 Playwright、Axe、离线应用壳、受控真实问账、生产服务器产物及真机安装态 PWA 更新验收全部通过
...
当前状态：共享纯统计、前端薄包装、统计范围/分类/趋势/drilldown 回归，以及 query plan、stats envelope、grounded answer 严格契约已完成本地实现和自动化检查；M1 已在后续独立批次完成。
...
当前状态：Edge JWT、邮箱白名单、环境变量和 OpenAI client 已抽为公共模块；用户 JWT + publishable key + RLS 的完整稳定分页、白名单筛选、正式统计、商家分组、比较和最多 500 条安全明细选择已完成本地实现与自动化检查。任一页或关键行字段校验失败都整体报错，不返回部分统计；后续 M3 兼容性修正仅将历史未知/空分类按现有规则归一为 `其他`。M1 本身未新增 `fox-chat` 或数据库写路径，M2 已在后续独立批次完成。
...
当前状态：`fox-chat` 第一次 AI、四类严格 intent union、V3.0 交易清洗复用、normalized query plan 校验、白名单澄清/不支持 key 和强制记账/问账纠错已完成本地实现与自动化检查。M2 独立批次本身不接受非空连续追问 context、不调用 M1 数据读取或第二次 AI；这些能力已在后续 M3 独立批次实现。
...
当前状态：完整 RLS 查询、完整统计包、最多 500 条五字段明细、第二次 AI、严格 grounded answer、metric/evidence refs 校验、服务端数字替换、第二次 AI 失败时统计卡降级、normalized 内存连续追问、前端问账卡、代码渲染依据及白名单账单筛选跳转已完成实现、自动化回归和受控人工验收。`fox-chat` 与 V3.1 静态前端均已部署。
...
当前状态：统一设计 token、共享页面头/指标卡/区块、登录与五个主路由表现迁移、品牌橙与支出语义色分离、同步缓存行数/最近成功时间/失败原因/手动重试、可见 focus、`aria-live`、`aria-busy`、reduced-motion 和 Safe Area 适配已完成。未修改同步算法、交易 API、统计口径、数据库 schema、RLS 或 AI 数据边界；M4 已通过验收并以提交 `3a7b11b` 推送发布。
...
当前状态：新增 15 条系统 Chrome 桌面、Pixel 7、Axe 与 PWA Playwright 流程；修复移动端输入框失焦导致发送按钮布局跳动、设置页原生 CSV 控件在窄屏撑大页面的问题，并将品牌/财务/导航颜色调整到 WCAG AA；320px 下五个主页面均无横向溢出。受控真实账号只读问账已返回可信范围、代码统计和五字段依据，未触发写入。`lint`、`typecheck`、33 个测试文件/157 项测试、`build`、Playwright、`verify:v3`、依赖审计、生产部署、服务器产物及真机安装态 PWA 更新验收已全部通过。
