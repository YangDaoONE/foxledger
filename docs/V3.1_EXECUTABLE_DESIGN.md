...
- 文档性质：已确认产品边界后的实施设计；M0 已完成本地代码和自动化检查，M1–M5 尚未实现
- 适用仓库：`D:\fox\foxledger` Web/PWA
- 当前实施状态：M0“共享统计与契约测试”已在本地完成，尚未发布；生产站点仍为 V3.0
...
当前状态：共享纯统计、前端薄包装、统计范围/分类/趋势/drilldown 回归，以及 query plan、stats envelope、grounded answer 严格契约已完成本地实现和自动化检查；未开始 M1。
